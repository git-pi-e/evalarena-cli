...
from ..utils.completions import complete_chat_models
...
async def stream_chat_response(
    ...
    try:
        ...
        async with client.stream("POST", "/api/chat/chat/", json=payload, timeout=httpx.Timeout(60.0)) as response:
...
def create_live_display(
    ...
    table = Table(title="🤖 Multi-Model Chat Comparison", show_header=True, header_style="bold blue", border_style="blue", expand=True)
    for model in models:
        model_name = model['name'][:20] + "..." if len(model['name']) > 20 else model['name']
        table.add_column(f"{model_name}\n({model['creator']})", style="white", no_wrap=False, vertical="top")
    progress_row = []
    for model in models:
    ...
    for model in models:
        ...
        if not response_text:
            ...
            display_text = response_text[:500] + ("\n\n[... response truncated ...]" if len(response_text) > 500 else "")
...
def print_final_results(models: List[Dict[str, str]], responses: Dict[str, str]):
    ...
    for i, model in enumerate(models, 1):
        panel = Panel(responses.get(model['id'], "No response"), title=f"[bold]{i}. {model['name']}[/bold] ([blue]{model['creator']}[/blue])", border_style="blue", padding=(1, 2))
        console.print(panel)
        console.print()
...
async def chat_with_multiple_models(
    ...
    async with create_client() as client:
        tasks = [
            stream_chat_response(client, prompt, model['id'], responses, progress_status)
            for model in selected_models
        ]
        if show_progress:
            with Live(create_live_display(selected_models, responses, progress_status), refresh_per_second=2, console=console) as live:
...
def chat_command(
    ...
    models: Optional[str] = typer.Option(None, "--models", "-m", help="Comma-separated list of model IDs to compare", autocompletion=complete_chat_models),
    set_models: Optional[str] = typer.Option(None, "--set-models", help="Persist default chat models (comma-separated). Use --clear-models to remove.", autocompletion=complete_chat_models),
    clear_models: bool = typer.Option(False, "--clear-models", help="Clear default chat models from config"),
    list_models: bool = typer.Option(False, "--list", help="List available chat models"),
    no_progress: bool = typer.Option(False, "--no-progress", help="Disable live progress display")
) -> None:
